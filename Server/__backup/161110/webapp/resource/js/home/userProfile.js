$(document).ready(function() {
  ...
  $("#btnModifyProfile").bind('click', function() {
    ...
    // 정보입력 input 활성화
    $(".user-profile-wrapper input").removeAttr('readonly');
    $(".user-profile-wrapper input[name='profileID']").prop('readonly', true);
  ...
  $("#btnCancelProfile").bind('click', function() {
    ...
    // 정보입력 input 비활성화
    $(".user-profile-wrapper input").prop('readonly', true);
  ...
  $("#btnChangeProfile").bind('click', function() {
    ...
    if(pwd.val() != "") {
      if(pwd.val().length < 4 || pwd.val().length > 8) {
        pwd.val("");
        txtPwd.text("* 비밀번호는 4~8자리입니다.").css('color', 'red');
      } else {
        txtPwd.text("");
      }
      if(pwd.val() != pwdCheck.val()) {
        pwdCheck.val("");
        txtPwdCheck.text("* 입력한 비밀번호를 다시 확인해주세요.").css('color', 'red');
      } else {
        txtPwdCheck.text("");
      }
    }
    ...
    if(name.val().length > 16) {
      name.val("");
      txtName.text("* 이름은 최대 16자까지 입력가능합니다.").css('color', 'red');
    } else { // 이름 길이가 적절하면
      // 정규식 체크
      ...
      if(!regEx.test(name.val())) {
        name.val("");
        txtName.text("* 이름은 한글, 영문으로 2~16자입니다.").css('color', 'red');
      } else {
        txtName.text("");
      }
    }
    ...
    // 주소 체크
    if(address.val() == "") {
      txtAddress.text("* 주소를 입력해주세요.").css('color', 'red');
    } else {
      txtAddress.text("");
    }
    ...
    if(!regEx.test(phoneNo.val())) {
      phoneNo.val("");
      txtPhoneNo.text("* 전화번호는 10~11자리 숫자입니다.").css('color', 'red');
    } else {
      txtPhoneNo.text("");
    }
